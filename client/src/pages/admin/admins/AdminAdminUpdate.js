...
import { GetSingleAdminsAction, UpdateAdminAction } from '../../../redux/actions/Admin/AdminsAction';
import LoadingTable from '../../../components/LoadingTable'
import { toast } from 'react-toastify';
import { UPDATE_ADMINS_RESET } from '../../../redux/constants/Admin/AdminsConstant';
import { AdminInitialPermissions, UncombineAdminPermissions } from '../Permissions';
...
const AdminAdminUpdate = () => {
    ...
    const { loading, admin, isUpdated, message } = useSelector((state) => state.admins);
    const { id } = useParams();
    ...
    useEffect(() => {
        dispatch(GetSingleAdminsAction(id));
    }, [dispatch, id]);
    useEffect(() => {
        if (admin) {
            setData({
            ...
            const uncombinedPermissions = UncombineAdminPermissions(admin.permissions);
    ...
    const UpdateAdmin = async (event) => {
        ...
        await dispatch(UpdateAdminAction(id, formData))
    }
    useEffect(() => {
        if (isUpdated && isUpdated === true) {
            toast.success(message, { theme: "colored" })
            dispatch({ type: UPDATE_ADMINS_RESET })
            navigate('/admin/admin')
        }
    }, [dispatch, navigate, isUpdated, message, id]);
    return (
        <div className="content-wrapper">
            <MetaData title="Admin - Update Admin" />
...
export default AdminAdminUpdate
