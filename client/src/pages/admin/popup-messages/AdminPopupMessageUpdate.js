...
import { GetSinglePopupMessageAction, UpdatePopupMessageAction } from '../../../redux/actions/Admin/PopupMessageAction';
import { UPDATE_POPUP_MESSAGE_RESET } from '../../../redux/constants/Admin/PopupMessageConstant';
import { GetClientsAction } from '../../../redux/actions/Admin/ClientAction';


const AdminPopupMessageUpdate = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { id } = useParams();
    const { loading, isUpdated, message, popupMessage } = useSelector((state) => state.popupMessages);
    const { clients } = useSelector((state) => state.clients);

    useEffect(() => {
        dispatch(GetSinglePopupMessageAction(id));
    }, [dispatch, id]);



    const [data, setData] = useState({
        client_id: popupMessage?.client_id,
        content: popupMessage?.content,
        trigger_event: popupMessage?.trigger_event,
        visibility_start: popupMessage?.visibility_start,
        visibility_end: popupMessage?.visibility_end,
        status: popupMessage?.status,
    })
    ...
    const UpdatePMFunction = async (event) => {
        ...
        await dispatch(UpdatePopupMessageAction(id, formData))
    }
    useEffect(() => {
        dispatch(GetClientsAction())
        if (isUpdated && isUpdated === true) {
            toast.success(message, { theme: "colored" })
            dispatch({ type: UPDATE_POPUP_MESSAGE_RESET })
            navigate('/admin/popup/messages')
        }
    }, [dispatch, navigate, isUpdated, message]);
    return (
        <div className="content-wrapper">
            <MetaData title="Admin - Update Popup Messages" />
                    ...
                    <div className="col-12">
                        <div className="card">
                            <div className="card-header border-bottom ">
                                <h4 className="card-title">Update Popup Messages </h4>
                            </div>

                            {
                                loading ? <LoadingTable /> : (
                                    <form method="post" onSubmit={UpdatePMFunction} className='p-2'>
                                        <div className='row'>
                                            <div className='col'>
...
export default AdminPopupMessageUpdate
