...
export const GetSubscriptionCancellationAction = () => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${BaseURL}/api/admin/subscription/cancellation`, {
            headers: {
...
export const UpdateSubscriptionCancellationAction = (id, form) => async (dispatch) => {
    try {
        ...
        const token = localStorage.getItem('token');
        const { data } = await axios.post(`${BaseURL}/api/admin/subscription/cancellation/update/${id}`, form, {
            headers: {
                'Content-Type': 'application/json',
                "Authorization": `Bearer ${token}`
            }
...
export const DeleteSubscriptionCancellationAction = (id) => async (dispatch) => {
    try {
        ...
        const token = localStorage.getItem('token');
        const { data } = await axios.get(`${BaseURL}/api/admin/subscription/cancellation/delete/${id}`, {
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`
            }
